...
package jis
{
     ...
     *  </ul> 
     */
    public class STLTextField extends DisplayObjectContainer
    {
		private var mHtml:Boolean;
		...
		private var mQuadDic:Dictionary;
        ...
        public function STLTextField(width:int, height:int, text:String, html:Boolean=false, fontName:String="Verdana",
        ...
        private function createRenderedContents():void
        {
			...
			if(mHtml)
			{
				var aStrList:Vector.<String> = new Vector.<String>();
				var herfStrList:Vector.<String> = new Vector.<String>();
				
				var str:String = sNativeTextField.htmlText;
				var xml:XML = new XML(str);
				var aNodes:XMLList = xml.descendants("A");
				for each(var aNode:XML in aNodes)
				{
					aStrList.push(aNode.children())
					herfStrList.push(aNode.@HREF.toString());
				}
				
				var startIndex:int=0;
				for(var aStrIndex:int=0;aStrIndex<aStrList.length;aStrIndex++)
				{
					var s:String = aStrList[aStrIndex];
					var index:int = sNativeTextField.text.indexOf(s,startIndex);
					startIndex = s.length;
					
					var startRec:Rectangle = sNativeTextField.getCharBoundaries(index);
					var endRec:Rectangle = sNativeTextField.getCharBoundaries(index+s.length-1);
					var hot:Rectangle = new Rectangle(startRec.x, startRec.y, (endRec.x-startRec.x+endRec.width), startRec.height);
					
					var hotPanel:Quad = new Quad(hot.width, hot.height, 0xf0f0f0);
					hotPanel.alpha = 0;
					hotPanel.x = hot.x;
					hotPanel.y = hot.y;
					hotPanel.addEventListener(TouchEvent.TOUCH, onLink);
					mQuadDic[hotPanel] = herfStrList[aStrIndex];
				}
			}
		...
		private function onLink(e:TouchEvent):void
		{
			var touch:Touch = e.getTouch(e.currentTarget as DisplayObject);
			if(touch && touch.phase == TouchPhase.ENDED)
			{
				navigateToURL(new URLRequest(mQuadDic[e.currentTarget]));
			}
		}
		...
		/** html mode **/
		public function get html():Boolean { return mHtml; }
		...
		/**
		 * 将普通文本框转换为支持html字段的文本框
		 * @param text 文本框
		 * @param hasDispose 是否销毁旧的
		 * @return 返回支持Html字段的STLTextField文本
		 */
		public static function textFieldConvertHtmlTextField(text:starling.text.TextField,hasDispose:Boolean = true):STLTextField
		{
			var result:STLTextField = new STLTextField(text.width,text.height,text.text,true,text.fontName,text.fontSize,text.color,text.bold);
			result.x = text.x;
			result.y = text.y;
			if(text.parent) text.parent.addChild(result);
			if(hasDispose) text.removeFromParent(true);
			return result;
		}
    }
}