package jis.effect.pageflip
{
	...
	public class PageFlipContainer extends Sprite
	{
		...
		/**@private*/
		public function PageFlipContainer(textures:Vector.<Texture>,bookWidth:Number,bookHeight:Number,bookCount:Number)
		{
			super();
			...
			ImagePage.BOOK_WIDTH = bookWidth;
			ImagePage.BOOK_HEIGHT = bookHeight;
			initPage();
		}
		/**初始化页*/
...
//			addEventListener(TouchEvent.TOUCH,onTouchHandler);
		...
		private function enterFrameHandler(event:Event=null):void
		{
			...
			{
				cacheImage.x = 0;
				cacheImage.texture = getTextureForPageNum(leftPageNum);
			...
			if(validatePageNumber(rightPageNum))
			{
				...
				cacheImage.texture = getTextureForPageNum(rightPageNum);
				quadBatch.addImage(cacheImage);
			}
			//渲染正在翻转的页面
			if(validatePageNumber(flipingPageNum))
			{
				...
				{
					var modeTexture:Texture = getTextureForPageNum(begainPageLocationX>=0? flipingPageNum:flipingPageNum+1);
					if(modeTexture){
						flipImage.texture = modeTexture;
						flipImage.anotherTexture = getTextureForPageNum(begainPageLocationX<0 ? flipingPageNum:flipingPageNum+1);
				...
				{
					var softTexture:Texture = getTextureForPageNum(flipingPageLocationX>=0? flipingPageNum:flipingPageNum+1);
					if(softTexture){
						flipImage.texture = softTexture;
						flipImage.readjustSize();
...
//					}
		...
		private function beginPage(point:Point):void{
		...
		private function movePage(point:Point):void{
		...
		private function endPage(point:Point):void{
			if(isDraging)
			{
				finishTouchByMotion(point.x);
				isDraging = false;
		...
		/**设置硬皮还是软皮*/
		protected function resetSoftMode():void
		{
		...
		/** 最大页码 */
		public function get pageTotalNum():int { return bookCount; }
		/**强制更新一次显示*/
		...
		public function gotoNextPage():void{
			if(leftPageNum < bookCount-1){
				var point:Point = new Point(bookWidth,bookHeight);
				Starling.juggler.tween(point,0.5,{"x":point.x-bookWidth,"onStart":beginPage,"onStartArgs":[point],"onUpdate":movePage,"onUpdateArgs":[point],"onComplete":endPage,"onCompleteArgs":[point]});
			}
		}
		
		protected function getTextureForPageNum(pageNum:int):Texture{
			if(pageNum > textures.length-1) return null;
			return textures[pageNum];
		}
